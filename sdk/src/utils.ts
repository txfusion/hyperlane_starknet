...
export function validateContractName(name: string): ValidationError[] {
  ...
  if (name.length > CONFIG.CONTRACT_NAME_VALIDATION.MAX_LENGTH) {
    errors.push({ type: "tooLong", maxLength: CONFIG.CONTRACT_NAME_VALIDATION.MAX_LENGTH });
  }

  // Check for forbidden characters
  const foundForbiddenChars = CONFIG.CONTRACT_NAME_VALIDATION.FORBIDDEN_CHARS.filter((char) => name.includes(char));
...
export function assertValidContractName(name: string): void {
  ...
  if (errors.length > 0) {
    throw new ContractError(ErrorMessages[CONFIG.ERROR_CODES.INVALID_INPUT], CONFIG.ERROR_CODES.INVALID_INPUT, {
      name,

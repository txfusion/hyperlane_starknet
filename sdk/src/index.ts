...
export const getCompiledContract = (name: string): ContractData => {
  try {
    return JSON.parse(readFileSync(findContractFile(name, "STANDARD"), "utf-8"));
  } catch (error: unknown) {
    if (error instanceof ContractError) throw error;
    throw new ContractError(ErrorMessages[CONFIG.ERROR_CODES.PARSE_ERROR], CONFIG.ERROR_CODES.PARSE_ERROR, {
      name,
...
export const getCompiledContractCasm = (name: string): CompiledContractCasm => {
  try {
    return JSON.parse(readFileSync(findContractFile(name, "COMPILED"), "utf-8"));
  } catch (error: unknown) {
    if (error instanceof ContractError) throw error;
    throw new ContractError(ErrorMessages[CONFIG.ERROR_CODES.PARSE_ERROR], CONFIG.ERROR_CODES.PARSE_ERROR, {
      name,
 ...
 */
function findContractFile(name: string, suffix: keyof typeof CONFIG.SUFFIXES): string {
  assertValidContractName(name);
  const mainPath = `${TARGET_DEV_PATH}/${name}${CONFIG.SUFFIXES[suffix]}`;

  if (!existsSync(mainPath)) {
    throw new ContractError(ErrorMessages[CONFIG.ERROR_CODES.FILE_NOT_FOUND], CONFIG.ERROR_CODES.FILE_NOT_FOUND, {
      name,
      suffix,
      path: mainPath,
    });
